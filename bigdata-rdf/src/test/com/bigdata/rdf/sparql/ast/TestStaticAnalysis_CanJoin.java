...
//        
//        final Predicate<?> p1 = new Predicate(new BOp[]{x,y});
...
//        final IPredicate<?> p2 = new Predicate(new BOp[]{y});
        ...
        final VarNode x = new VarNode("x");
        final VarNode y = new VarNode("y");
        final VarNode z = new VarNode("y");
        
        final StatementPatternNode p1 = new StatementPatternNode(x, y, z);
        final StatementPatternNode p2 = new StatementPatternNode(y, x, z);
    ...
    public void test_canJoinUsingConstraints_1step_productVar() {
        ...
        for (int i = 0; i < a.length; i++) {
            for (int j = i; j < a.length; j++) {
                ...
                assertTrue(StaticAnalysis.canJoin(t0, t1));
                assertTrue(StaticAnalysis.canJoin(t1, t0));
                if (t0 != t1) {
                    /*
                     * Test join path extension, but not when the vertex used to
                     * extend the path is already present in the join path.
                     */
                    assertTrue(StaticAnalysis.canJoinUsingConstraints(//
    ...
    public void test_canJoinUsingConstraints_multiStep_productVar() {
        ...
        for (int i = 0; i < path.length - 1; i++) {
            assertTrue(StaticAnalysis.canJoin(path[i], path[i + 1]));
        }
    ...
    public void test_canJoinUsingConstraints_p3_p4() {
         ...
         * if it were executed).
         */
        assertFalse(StaticAnalysis.canJoin(p3, p4));
        assertFalse(StaticAnalysis.canJoin(p4, p3));
        assertFalse(StaticAnalysis.canJoinUsingConstraints(//
    ...
    public void test_canJoinUsingConstraints_p5_p6() {
         ...
         * if it were executed).
         */
        assertFalse(StaticAnalysis.canJoin(p5, p6));
        assertFalse(StaticAnalysis.canJoin(p6, p5));
        assertFalse(StaticAnalysis.canJoinUsingConstraints(//
