...
public class TestSnapshotIndex extends TestCase2 {
    ...
    public void test_emptyIndex() {
        ...
        assertNull(ndx.getOldestSnapshot());
        
        assertNull(ndx.getNewestSnapshot());

        {
            try {
                assertNull(ndx.getSnapshotByReverseIndex(-1));
                fail("Expecting: " + IllegalArgumentException.class);
            } catch (IllegalArgumentException ex) {
                // ignore
            }
            // Legal.
            assertNull(ndx.getSnapshotByReverseIndex(0));
            assertNull(ndx.getSnapshotByReverseIndex(1));
            assertNull(ndx.getSnapshotByReverseIndex(2));
        }
    ...
    public void test_nonEmptyIndex() {
        ...
        assertEquals(sr2, ndx.getNewestSnapshot());
        assertEquals(sr0, ndx.getOldestSnapshot());

        {
            // Indexing backwards from the end.
            assertEquals(sr2, ndx.getSnapshotByReverseIndex(0));
            assertEquals(sr1, ndx.getSnapshotByReverseIndex(1));
            assertEquals(sr0, ndx.getSnapshotByReverseIndex(2));
        }
