...
/**
 * A checkpoint record is written each time the hash index is flushed to the
 * store.
 * 
 * @todo Update for hash table, potentially using the same checkpoint record for
 *       the B+Tree and hash tables.
 *       <p>
 *       Note: In order to create a btree use
 *       {@link BTree#create(IRawStore, IndexMetadata)} to write the initial
 *       {@link IndexMetadata} record and the initial check point on the store.
 *       It will then load the {@link BTree} from the {@link Checkpoint} record
 *       and you can start using the index.
 * 
 * @author <a href="mailto:thompsonbry@users.sourceforge.net">Bryan Thompson</a>
 * @version $Id: Checkpoint.java 3318 2010-07-27 18:36:58Z thompsonbry $
 */
public class HTableCheckpoint implements Externalizable {
     ...
     */
    public HTableCheckpoint(final IndexMetadata metadata ) {
     ...
     */
    public HTableCheckpoint(final IndexMetadata metadata, final HTableCheckpoint oldCheckpoint ) {
...
//     *            The btree.
//     */
//    public HTableCheckpoint(final BTree btree) {
//        
...
//    }
    ...
    private HTableCheckpoint(final long addrMetadata, final long addrRoot,
...
//        assert addrMetadata != 0L;
     ...
     */
    public static HTableCheckpoint load(final IRawStore store, final long addrCheckpoint) {
        ...
        final HTableCheckpoint checkpoint = (HTableCheckpoint) SerializerUtil.deserialize(buf);
