 ...
 * 
 * Comparison operations that accept a starting offset are used when the
 * byte[]s are known to share a leading prefix that may be skipped during
 * comparison.
 * 
 * Comparison operations that accept a starting offset and length are used
 * when immutable keys are stored in a single byte[] and an index into
 * starting positions in that byte[] is maintained.
 * 
 * FIXME write JNI utilities to support comparison of unsigned bytes. this
 * should be a big performance win if we do not need to marshall the java
 * byte[] into C.  What C compiler should I be using for this? use register
 * variables for loops.
 * 
