 ...
 * {@link Node}. When a {@link Node} is split, the original {@link Node} is
 * retained, a new rightSibling of the node is created, and a separatorKey is
 * lifted into the parent. The splitIndex is the index of the first key to be
 * moved to the rightSibling. The separatorKey defines the lowest key value that
 * may enter into the rightSibling and may be either one of the keys from the
 * node that is being split or the insert key itself. Unlike a leaf split, the
 * separatorKey for a node split is not found in either the original node or its
 * new rightSibling after the split (hence, it is "lifted" into the parent vs
 * being copied into the parent).
 * 
 ...
 * @version $Id$
 */
public interface INodeSplitRule {
    ...
    /**
     * The separatorKey to be lifted into the parent.
     * 
     * @return
     */
