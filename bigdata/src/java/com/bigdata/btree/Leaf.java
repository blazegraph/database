...
public class Leaf extends AbstractNode<Leaf> implements ILeafData {
     ...
     * {@link #getKeyCount()} < {@link #minKeys()}). This arises when the
     * {@link IndexSegmentPlan} was generated based on an overestimate of the
     * actual range count. See {@link IndexSegmentBuilder} for details.
     */
...
//        this.maxKeys = isLeaf() ? branchingFactor : branchingFactor - 1;
    ...
    protected Leaf(final AbstractBTree btree, final long addr,
        ...
        if (addr == 0L && this instanceof ImmutableEmptyLastLeaf) {

            /*
             * Note: This is a mock right-most child of a node generated by an
             * index segment build based on an overestimate of the #of tuples.
             */
...
//        }
    ...
    public boolean dump(final Level level, final PrintStream out,
        ...
        if ((btree.root != this)
                && (
                        (nkeys == 0 && (!(this instanceof ImmutableEmptyLastLeaf)))
                        || 
                        (!(btree instanceof IndexSegment) && (nkeys < minKeys)))
                        ) {
            /*
             * Min keys failure.
             * 
             ...
             * 
             * Note: A mock leaf is automatically materialized as the right-most
             * child of a Node when the childAddr==0 and the B+Tree is an
             * IndexSegment. This handles an edge case in the index segment
             * build when insufficient tuples are available to populate the
             * IndexSegment according to the plan.
             */
