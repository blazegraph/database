...
public class WriteBlock {
	...
	public void addWrite(long diskAddr, byte[] buf, int size) {
		...
		if (m_directWrite) {
			try {
				...
				// cat.info("at: " + diskAddr + ", length: " + size);
			} catch (Exception e) {
	...
	void placeEntry(WriteEntry entry) {
		if (m_head == null) {
			...
			if (tail == null) {
				entry.m_next = m_head;
				m_head = entry;
			} else if (tail.m_diskAddr == entry.m_diskAddr) {
				tail.m_buf = entry.m_buf; // use updated buffer
	...
	static class WriteEntry {
		...
		void doWrite(RandomAccessFile file) {
			...
			if (m_writeCount++ > 0) {
				throw new Error("Write Block written more than once: " + m_writeCount);
			}

			try {
				file.seek(m_diskAddr);
				file.write(m_buf);
				// cat.warning("at: " + m_diskAddr + ", length: " + m_buf.length);
			} catch (Exception e) {
