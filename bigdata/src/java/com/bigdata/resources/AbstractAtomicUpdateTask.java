...
public abstract class AbstractAtomicUpdateTask<T> extends
    ...
    /**
     * The {@link UUID} assigned to the scale-out index whose view will be
    ...
    /**
     * Verifies that the view is a view of the same index whose {@link UUID} was
     * specified to the ctor. This is used to detect drop/add sequences where a
     * scale-out index exists with the same name but a different index
     * {@link UUID}. Atomic update tasks check for this and disallow the update
     * in order to prevent incorporation of the old index state into the new
     * index.
     * 
     * @param view
     ...
     */
    protected void assertSameIndex(final BTree view) {
        
        if (!indexUUID.equals(view.getIndexMetadata().getIndexUUID())) {
