...
//            final long commitCounter, final long commitTime,
	...
	class WORMOutputStream extends IPSOutputStream {
		...
		public long getAddr() {
			...
			final long addr = WORMStrategy.this.write(bb);
			...
			final long hdrAddr = WORMStrategy.this.write(hbb);
		...
		public void write(int b) throws IOException {
			...
			if (m_cursor == WORM_STREAM_BUFFER_SIZE) {
				...
				final long waddr = WORMStrategy.this.write(bb);
				if (m_header == null) {
...
//			}
			...
			if (isStream) {
				...
				m_hbb = WORMStrategy.this.read(am.toAddr(nblocks * 8, am.getOffset(addr))); // 8 bytes size of long
				
				// read first block
				assert m_hbb.hasRemaining();
				m_bb = WORMStrategy.this.read(m_hbb.getLong());
			} else {
				m_hbb = null;
				
				if (nbytes > 0)				
					m_bb = WORMStrategy.this.read(addr);	
				else
